...
Mosaic large GeoTiff files using GDAL and Python extension package with less time
...
class LargeGeoTiffMosaic:
    ...
    The C/C++ Python extention package written by Dr. Debabrata Ghorai to convert 
    a float-type numpy array to integer-type numpy array fastly.
    """
    ...
    def __init__(self):
        ...
        self.mylib = ctypes.CDLL(
            os.path.join(PRJ_FLD, 'artifacts', 'extension', 'library_windows64bit', 'floattoint.dll')
        )
        self.Multiply_and_Convert_Values = self.mylib.Multiply_and_Convert_Values
        self.Multiply_and_Convert_Values.restype = None
        self.Multiply_and_Convert_Values.argtypes = [
            ctypes.c_int, ctypes.c_int, numpy.ctypeslib.ndpointer(ctypes.c_float), 
            ctypes.c_float, numpy.ctypeslib.ndpointer(ctypes.c_int)
        ]

    def float_raster_to_int_raster(self, tif_file_list, outf):
        """
        ...
        for f in tif_file_list:
            ...
            out_arr[
                numpy.logical_or(out_arr <= -2147483648, out_arr > 2147483647)
            ] = self.nodata
    ...
    def mosaic_raster_tiles(self, path, outf):
        ...
        new_file_list = self.float_raster_to_int_raster(tif_file_list, outf)
        ...
        subprocess.call(
            f"gdal_translate -of GTiff -co COMPRESS=LZW {vrt_output} {tif_output}"
        )
